...
Or interactively:
#+begin_src lisp :results replace :wrap src lisp :exports both
  (ql:quickload :macs/tests)
...
#+begin_src lisp
((T NIL)
 (#<PASS ((IS (TYPEP (MAKE-FIXTURE-PROTOTYPE EMPTY NIL) 'FIXTURE-PROTOTYPE))
          (IS
           (TYPEP
            (MAKE-FIXTURE TFIX
                NIL
          ...
          (LET ((FX1
            ...
            (IS
             (WITH-FIXTURE (A B C)
                 FX1
               (NOT (MEMBER 'NIL (MAPCAR #'= (LIST 1 2 3) `(,A ,B ,C))))))))>
  #<PASS ((IS (TEST-PASS-P (IS (= (+ 2 2) 4)))) (IS (WHEN T T))
          (IS
           (TEST-PASS-P
            (SIGNALS (ERROR T)
  ...
  #<PASS ((IS
           (PROGN
            (IN-READTABLE *MACS-READTABLE*)
            (TYPEP (LAMBDA (A1) `(,A1 ,A1 ',A1 ,@A1)) 'FUNCTION))))>
  #<PASS ((WITH-GENSYMS (A B C)
            (LET ((A 1) (B 1) (C 2))
              (IS (AND (= A B) (NOT (= C A)) (= (+ A B) (* C B)))))
            (IS (NOT (EQUALP (MAKE-GENSYM 'A) (MAKE-GENSYM 'A))))
            (IS (EQ (ENSURE-SYMBOL 'TESTS MACS.TESTS) 'TESTS))
            (IS (EQ 'FOO (FORMAT-SYMBOL MACS.TESTS ~A 'FOO)))
            (IS (EQ (MAKE-KEYWORD 'FIZZ) FIZZ))))>
  #<PASS (Test MACS.STR)> #<PASS (Test MACS.STR)> #<PASS (Test MACS.STR)>
  #<PASS (Test MACS.STR)> #<PASS (Test MACS.STR)> #<PASS (Test MACS.STR)>
  #<PASS (Test MACS.STR)> #<PASS (Test MACS.STR)> #<PASS (Test MACS.STR)>
  #<PASS (Test MACS.STR)> #<PASS (Test MACS.STR)>
  #<PASS ((LET ((*STANDARD-INPUT*
            ...
            (IS
             (STRING= foobar
                      (COMPLETING-READ nothing:  TCOLL HISTORY 'THIST DEFAULT
                                       foobar))))
          (IS (EQ (MAKE-SHORTY test) t))
          (DEFVAR %OPTS
            (MAKE-OPTS '(NAME FOO GLOBAL T DESCRIPTION bar)
                       '(NAME BAR DESCRIPTION foo)))
          (DEFVAR %CMDS (MAKE-CMDS `(NAME BAZ DESCRIPTION baz OPTS ,%OPTS)))
          (IS (AND %OPTS %CMDS)))>))
#+end_src
